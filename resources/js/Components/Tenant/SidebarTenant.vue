    ...
    <template v-for="(item, index) in menu">
      ...
      <template v-else>
        <v-list-group
          v-model="item.active"
          :key="Number(item.id)"
          :value="item.title + '-' + (item.id + 1000)"
          :title="item.title"
          @click="OpenSubMenu(item.title + '-' + (item.id + 1000))"
        >
          ...
          <v-list-item
            v-for="(sub, num) in item.newSubMenu"
            :key="num"
            :value="sub.title + (num + 1000 + index)"
            :title="sub.title"
            @click="VisitPage(sub.route)"
            :active="isActiveSubMenu(sub)"
            :prepend-icon="
              isActiveSubMenu(sub) ? 'mdi:mdi-rhombus-medium' : sub.icon
            "
            exact
          >
          </v-list-item>
