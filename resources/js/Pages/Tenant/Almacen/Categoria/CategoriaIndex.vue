...
const $props = defineProps(['flash', 'categorias']);
...
function Listar() {
    Inertia.reload({
        only: ['categorias'],
        onSuccess: () => {
            dataCategoria.value = $props.categorias;
        }
    })
}
...
function SubirCategoriaExcel() {
    formFileExcel.post(route('importexcel.categoria'), {
        forceFormData: true,
        onSuccess: () => {
...
<template>
    <TenantLayout title="Categorias" titleBread="Listado de categorias">
        <template #itemBreadCrumb>
            <v-breadcrumbs :items="['Almacen', 'Categorias', 'Lista']" class="pa-0"></v-breadcrumbs>
        </template>
        <CardView>
            <template #title>
            ...
            <template #content>
                <v-table density="comfortable" hover>
                    ...
                    <tbody>
                        <template v-if="dataCategoria.length > 0">
                            <tr v-for="(item, index) in dataCategoria" :key="index">
        ...
        <AddEditCategoria v-model:show="modal" v-model:formulario="formulario" @addingCategoria="Listar" />
        <v-dialog v-model="modalExcel" max-width="550" persistent>
            <v-form @submit.prevent="SubirCategoriaExcel">
                <v-card append-icon="mdi:mdi-file-excel-outline" title="Subir el Excel"
                    ...
                    <v-card-text>
                        <v-row>
                            <v-col cols="12" md="12">
                                <v-btn :href="route('modeloexcel.categoria')" block variant="outlined" color="orange">
                        ...
                        <v-row>
                            <v-col cols="12" md="12">
                                <v-file-input @input="formFileExcel.fileCategoria = $event.target.files[0]"
